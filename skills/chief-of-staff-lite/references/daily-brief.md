...
For each source needed by the procedure:

- `connected`: read it only if it is actually available in this session, and only within its configured scope.
...
Apply the authored priority and escalation logic. Consider strategic impact, urgency, reversibility, and where the CEO's involvement changes the outcome. Ordered priorities break otherwise comparable ties, but a material urgent risk may outrank a higher-listed goal. A blanket stakeholder escalation means surface the evidence and uncertainty, not manufacture a risk.

Prepare what the adapted procedure calls for: a grounded decision, reconciled follow-through, a meeting position, or another useful piece of assistance. Do not stop at telling the CEO to investigate everything you could have established from the approved evidence. Put routine or resolved items through the procedure's omission rules.
...
Check the result against the authored procedure and confirmed profile:
