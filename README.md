...
Reading-time choices are checked, not treated as vague style hints: the seven-section core brief allows up to 300 words for 3 minutes, 550 for 5 minutes, and 1,000 for 10 minutes. Optional follow-up drafts sit in their own final section, do not count toward the core budget, and are limited to two. Draft length is chosen for the purpose, audience, and CEO's observed writing patterns; there is no arbitrary universal word cap. Core limits are maximums, not targets, so a quiet day stays short while a longer preference creates room for more supported synthesis and preparation.

The skill uses only approved source scopes available in the current session. Setup normally sees only which capability names appear available; it does not test account access or connect tools. If drafts are enabled and email appears available, setup can separately ask permission for a one-time, read-only calibration using exactly the CEO's last 20 Sent emails. It never uses received mail for this analysis. The personalized private reference retains three representative sent-email excerpts, with signatures, quoted history, addresses, and metadata removed. The daily brief checks actual access and reports missing coverage. It can prepare useful follow-up drafts but never sends them automatically.
...
├── references/email-drafting.md      # Private voice and purpose-based length guidance
...
Run `python3 skills/chief-of-staff-lite/scripts/configure_skill.py --check-bundle` to validate the bundle. Preview and apply consume JSON through `--config-stdin`. Preview writes nothing, and the approval hash binds the destination, current state, proposed skill, and bundled resources. Existing local skills atomically update the marked configuration block and personalized email-drafting reference. Hosted packages use a fixed allowlist of the five runtime files and atomic ZIP replacement. Daily output is passed through `--check-brief-stdin` with the configured reading-time and draft settings before delivery; that check verifies the core word ceiling, seven-section order, and optional-draft placement and count without imposing a draft word cap.
